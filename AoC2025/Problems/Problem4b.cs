...
{
    internal class Problem4b : IProblem
    {
        ...
        public async Task Run()
        {
            ...
            var removedPaperRollsTotal = 0;
            var removedPaperRollsInIteration = 0;

            do
            {
                removedPaperRollsInIteration = 0;
                ...
                for (int row = 0; row < scenario.Rows; row++)
                {
                    for (int column = 0; column < scenario.Columns; column++)
                    {
                        ...
                        if (sum < 4)
                        {
                            removedPaperRollsTotal += 1;
                            removedPaperRollsInIteration += 1;
                            paperConfiguration[row * scenario.Columns + column] = 0;
                        }
                    }
                }
            } while (removedPaperRollsInIteration > 0);

            Console.WriteLine(removedPaperRollsTotal);
        }
